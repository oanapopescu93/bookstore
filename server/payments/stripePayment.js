...
const MINIMUM_AMOUNT_USD = 50 // 50 cents = 0.50 USD

stripePayment.post("/api/stripe", jsonParser, (req, res, next) => {
    const { cardNumber, month, year, cvv, description, products, amount } = req.body
    ...
    if (amount) {        
        ...
        lineItems.forEach((item, index) => {
            metadata[`product_${index + 1}`] = `${item.title}: ${item.quantity} x ${item.price / 100} USD`
        })
        ...
        try {
            createNewCustomer({ description: "Bookstore customer" }).then((res1)=>{
                if(res1){
                    customer = res1
